...
@pytest.mark.skipif(not DEMO_BUNDLE.is_dir(), reason="canonical replay bundle is unavailable")
def test_judge_theme_keeps_all_result_workspaces_and_peer_domain_views(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    result = _canonical_result()
    monkeypatch.setattr(
    ...
    for key in (
        "judge_workspace_shell",
        "judge_panel_overview",
        "judge_panel_evidence",
        "judge_panel_market",
        "judge_panel_reasoning",
        "judge_panel_report",
        "judge_domain_workspace",
    ):
    ...
    assert "background:rgba(255,255,255,.72)!important" in source
    assert "rgba(235,227,255,.92)" in source
    assert 'judge_domain_workspace [role="tab"]:nth-child' not in source
    assert "不代表主次或隶属关系" in source
