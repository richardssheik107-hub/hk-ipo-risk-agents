...
```text
fixed-journal gated M1 = 15/30 =50.00%
fixed-journal gated M2 = 24/48 =50.00%
offline M1 = 10/30
offline M2 = 18/48
Development20 offline M1 = 14/50 =28.00%
Development20 offline M2 = 28/98 =28.57%
```

Batch008–010 没有新的 remote provider 调用，所以最后一个真实 fresh checkpoint 仍是 Batch005：
...
**不要把 `15/30,24/48` 写成 fresh-provider 结果。**
...
只恢复严格完整 rank-1-to-5 表格在原 physical page 上的真实 Evidence，并保留既有风险判断、等级、状态、分数和 Calculation。效果：fixed-journal gated 到 `15/30,24/48`；Development20 offline 到 `14/50,28/98`，extra10 无回退。
...
```text
1. remaining deterministic / numeric extraction
2. risk conversion after consumed Evidence
3. exact page / anchor Evidence binding
...
历史 Batch001–010 只保留单一 `V046_ROLE_B_EXPERIMENT_LEDGER.md` + machine-readable reports + Git history；不要再为每个小 batch 往 `docs/` 主目录堆新的当前计划。
