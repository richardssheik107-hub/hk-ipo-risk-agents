...
> 当前状态：进行中
> 当前阶段：A线 A2.5——12份正式主表页核对完成，Retriever命中率未过门槛，暂停A3并进入排序整改；B线建立赛事数据清单
> 当前PR：无A线未合并PR
...
> A2.5自动测试及12份正式主表页核对已完成，但Top 5命中率未过门槛。
> 暂停第3棒，只允许进行一轮Retriever正式主表排序整改并复测。
...
**状态：A2.5未通过，禁止开始；等待Retriever整改及同批样本复测。**
...
|  | A3 财务抽取 | 禁止提前开始 | A2.5未达到两项≥75% | 等待Retriever整改及复测 |
...
- 现金Top 5命中率36.36%，经营现金流Top 5命中率63.64%；
- A2.5未通过，暂停A3，只进行Retriever正式主表排序整改和同批复测。
