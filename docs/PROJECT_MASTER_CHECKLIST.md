...
| 下一规划版本   | v0.4市场预测（V04-1/V04-2已合并，V04-3等待Review） |
...
- V04-1 Market Foundation与V04-2文档风险快照/100项特征契约已合并；V04-3已实现
  20项上市前market manifest与120项高层join并等待Review；Market Agent和生产行情
  适配器尚未接入。
...
| v0.4.0 | 文档风险能否连接上市后真实表现 | 行情、标签、文档特征契约、Market Agent、模型 | V04-1/V04-2 MERGED / V04-3 IMPLEMENTED PENDING REVIEW |
...
保护的数据集join。V04-3已实现严格point-in-time的20项market manifest和120项高层join，
等待Review；生产真实行情覆盖仍有限。这不代表完整v0.4完成；Market Agent及模型仍按后续阶段
推进。专项政策见`docs/research/V04_MARKET_FOUNDATION.md`和
`docs/research/V04_DOCUMENT_MARKET_FEATURE_CONTRACT.md`、
`docs/research/V04_PRELISTING_MARKET_FEATURES.md`。

| **编号** | **任务** | **主要输出** |
|:---|:---|:---|
| V4-1 | Market Foundation | 证券资格、行情契约、上市后标签与时间边界 |
| V4-2 | Document-to-Market Feature Contract | 文档风险快照、稳定manifest、dataset join与blind保护 |
| V4-3 | Pre-listing Market Feature Engine | 恒指、行业、近期IPO、波动率和市场活跃度 |
| V4-4 | 模型基线 | Rule、Logistic和LightGBM |
| V4-5 | 时间外验证 | 2020—2023开发、2024验证 |
| V4-6 | 最终盲测 | 2025一次性评估，不再调参 |
...
当前统一阶段：**v0.3 Released / Frozen**。V04-1 Market Foundation与V04-2
Document-to-Market Feature Contract已合并；V04-3上市前市场特征引擎已实现并等待
Review，生产真实行情覆盖仍有限；生产市场预测模型仍未开始。
...
| v0.4 Market Prediction | V04-1/V04-2 MERGED / V04-3 IMPLEMENTED PENDING REVIEW | 100项文档特征+20项上市前市场特征契约已实现；生产数据adapter和模型仍为后续Gate |
