...
- v0.4.6 offline / shadow / gated、LLM journal、Financial high-recall adapter、retrieval/risk waterfall；
- persisted-result read-only Evidence auditor，可检查 Evidence、页码/文本、bbox、Calculation、Index、provenance、leakage 和 determinism。

未关闭：

- M1 当前 fixed-10 仅 `23.33%`，目标 `>=80%`；
- M2 当前 fixed-10 仅 `18.75%`，目标 `>=85%`；
- v0.4.6 尚未形成完整同身份 fixed-10 实测；
- Candidate / LLM / Builder / Reconciliation / Verifier 生命周期 Trace 仍不完整；
...
- Role-D 1D / 5D / 20D / 60D 正式物化与 receipt；
- Role-D Development-selected v2 high-recall candidate。

未关闭：

- 关键风险原 PDF 精确高亮截图；
- 三个正式案例的完整 AI 报告与 M4 评审；
- D 模型 promote/retain 治理决议；
- 决议后 current-main strict revalidation 与 D→E final-three package；
...
| M5 frozen PR-F | Recall `4.35%`、F1 `7.69%` | current-main strict revalidation；如保留则限定业务主张 |
| M5 v2 candidate | Recall `52.17%`、F1 `42.11%`、PR-AUC `38.12%` | A promotion review、新 freeze 与 handoff |
...
目标：先证明每个 M1/M2 失分的最早失败阶段，不再把所有最终缺失都归为 `semantic_extraction_miss`。

执行：

1. 校验 fixed-10、Gold、代码、Prompt、provider、journal 身份；
2. 运行 matching v0.4.6 structured smoke；
3. 运行 offline / shadow / gated 同口径 fixed-10；
4. 对 persisted result 运行 read-only Evidence audit；
5. 补齐 parser、retrieval、LLM、builder、reconciliation、verifier、binding 瀑布；
6. 形成逐 Risk Unit / Evidence Unit 根因矩阵。
...
1. A 审核 frozen PR-F 与 v2 candidate；
2. 创建 promote / retain decision record；
3. 若晋升 v2，冻结 feature、model、alert policy，不再按 2024 调参；
4. current-main strict revalidation；
5. resume / fresh-directory determinism；
6. 生成 final-three label-free handoff；
7. 最终报告准确披露性能、alert 工作量和局限。
...
这些能力不混入 Existing-Gold M1/M2 分数：
...
退出条件：每项至少有一条真实案例、Evidence、Trace 和可展示产物；无法正式量化的能力标为 qualitative demonstration。

### 阶段 5 — 冻结、复验与一次性 Validation

1. B 代码、Prompt、Retriever、Schema、Verifier、Evaluator 冻结；
2. D 正式模型与 alert policy 冻结；
3. B 一次性运行 ALL 19 Validation；
4. D/C/E final artifact 固化；
5. latest-main CI、Blind、provenance、determinism、security 审计。

Validation 运行后不得回头调整 Prompt、Retriever、Verifier、模型、阈值或 evaluator。
...
```text
D model promotion review / release revalidation
...
- 不再固定“最多 2–4 轮”；改为根因覆盖、指标增益和停止条件；
- 不再 Runner-only；诊断、消融、代码审计和修复可以分别执行；
- 不再限制 Codex 只能读取两个 summary；允许读取与根因相关的完整代码、Trace 和 artifact；
- 不再禁止 Retriever 改造；允许 Development-only、无 Gold 泄漏、可消融的通用检索改造；
...
- 从本规则生效起，2025 Blind 输入和 outcome 都不用于优化；
...
- 需要使用 Blind 输入或 outcome；
- 模型 / transport 结构化合同无法在有界预算内稳定；
