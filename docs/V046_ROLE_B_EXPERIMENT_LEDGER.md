...
本文件是 Role-B fixed10 / fixed-journal 历史实验的单一人类可读总账。过去的逐批 Markdown 说明已从 `docs/` 主目录清理；完整 diff 仍由 Git history 保留，机器可读结果继续保留在 `reports/v046_role_b/`。
...
Fresh provider checkpoint 单独记录，因为 journal/provider identity 不同，并包含真实 runtime variance。最后一个 real fresh checkpoint 仍是 Batch005：M1 `11/30`、M2 `17/48`。Batch008/009 `network_calls=0`，不得把其 fixed-journal 结果描述为 fresh-provider gain。
...
- direct ranked concentration-table candidate：没有提升 canonical M1/M2，并使 supplier existence F1 `0.875 → 0.80`，已完整回滚；不得在没有新证据/新通用假设的情况下直接恢复。

历史 bounded audit 也已经排除 broad period candidate generation 和 broad Parser preservation 作为当时 active root。

## Current handoff to live execution

Batch009 结束时曾记录 `PAUSED_FOR_OWNER_REPLAN_AFTER_BATCH009`；该字段只表示当时 batch close 状态，**不是当前 live campaign controller**。

当前 live 执行以 `team/01_M1_M2_OWNER.md` 为准，当前 root order：
...
执行模式已经升级为 multi-root wide sprint / goal mode：多个 proven compatible roots 可同轮推进、独立 commit、bundle benchmark、partial revert，并在 meaningful gain 后扩大 Development。
...
full Development Gate 仍开放。fixed10 提升是诊断 evidence，不是 Competition Ready。

## Preservation policy

不再为每个新 batch 在 `docs/` 主目录堆单独历史说明。历史保存依赖：
