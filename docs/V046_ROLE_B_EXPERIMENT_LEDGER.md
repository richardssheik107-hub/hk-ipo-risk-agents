...
point. Large runtime directories, PDFs, provider responses, caches, and local
paths remain intentionally outside Git.
...
Only results sharing the frozen fixed-10 identity and the canonical replay
journal are compared below:

```text
Batch 003 floor  M1 10/30  M2 14/48
...
identities differ and they measure runtime LLM variance. They are not silently
substituted for fixed-journal results.
...
evidence, not competition readiness.
