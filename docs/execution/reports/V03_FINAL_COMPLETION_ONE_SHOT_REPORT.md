...
- `src/ipo_risk/agents/supervisor_v03.py`: stable deduplication and explicit revenue-semantics consistency handling;
...
- `app/presenters.py` and `app/streamlit_app.py`: Service-only v0.3 UI, domain views and Markdown/JSON downloads;
...
failure-degradation scenarios. The v0.3 view shows Financial, Legal and Business
risks, Evidence pages/text, Calculation details, Supervisor output, diagnostics,
governance status, and safe Markdown/JSON downloads. It imports no concrete Agent,
Parser, Provider, Predictor or Repository.

`V03ReportGenerator` returns ten deterministic sections including needs-review,
degradation and owner-waiver limitations. It makes no new risk decision.
...
python scripts/validate_competition_data.py    PASS
...
enhanced_v2 no-LLM real E2E                    completed / graceful
