...
def evaluate_development_cv_baselines(
    ...
    *,
    ...
    """Development-only diagnostic for Oracle cohorts lacking 2024 Gold."""

    if development.dataset_split.value != "development":
        raise ValueError("Oracle CV accepts Development only")
    x, y = _xy(development, "binary")
    class_counts = np.bincount(y, minlength=2)
    n_splits = min(folds, int(class_counts.min()))
    if n_splits < 2:
        raise ValueError("Development CV needs at least two rows in each class")
    splitter = StratifiedKFold(
        n_splits=n_splits, shuffle=True, random_state=PR_E_RANDOM_SEED
    )
    probability = np.zeros(len(y), dtype=float)
    raw_y = np.asarray(development.raw_return_5d, dtype=float)
    linear_prediction = np.zeros(len(y), dtype=float)
    ridge_prediction = np.zeros(len(y), dtype=float)
    ...
    metrics = {
        "logistic_regression": _classification_metrics(y, probability),
        "linear_regression": _regression_metrics(raw_y, linear_prediction),
        "ridge_regression": _regression_metrics(raw_y, ridge_prediction),
    }
    return tuple(
        BaselineEvaluation(
            model_family=family,
