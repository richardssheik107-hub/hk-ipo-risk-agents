"""Governed streaming EOD-store builder for the official 2020-2024 IPO cohort.

This module contains deterministic domain/data logic shared by PR-B CLIs.  The
``scripts/build_v04_ipo_eod_store.py`` file remains a thin command-line wrapper.
"""
...
def sha256_file(path: Path) -> str:
    """Return a deterministic SHA-256 for a file."""
...
def build_store(
    *,
    ...
    """Build or reuse a filtered, provenance-checked official-IPO EOD store."""
        ...
        for row in reader:
            ...
            if trade_date:
                min_trade_date = (
                    ...
                    else min(min_trade_date, trade_date)
                )
                max_trade_date = (
                    trade_date
                    if max_trade_date is None
                    else max(max_trade_date, trade_date)
                )
