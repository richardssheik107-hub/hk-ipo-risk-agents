...
`v1.0.0` 是本项目的比赛最终产品发布版。功能、运行时、Market-X、Frozen Model / SHAP、前端、能力证明和发布文档均已进入冻结/回归保护状态。最后一次前端收口把标准启动与评审启动统一到同一个 `app/streamlit_app.py` 工作台，避免两套界面漂移；`START_JUDGE_DEMO.*` 仅作为兼容入口保留。

“正式发布”表示产品版本完成并可用于比赛提交，不等于内部自定义 `COMPETITION_READY` 条件全部满足。本仓库不降低 G2 阈值，也不把较高的 offline 结果替代真实 provider-backed 结果。
...
正式 provider-backed Development 结果是 Real LLM gated。较高的 offline 结果仅保留为工程参考。
...
One canonical Streamlit workspace + compatibility judge launchers
...
最新产品界面收口提交 `006c7f3...` 的 `tests`、`Role D runtime`、`Team demo runtime` 均为 SUCCESS。
...
2. **Historical Governed IPO** — 对受治理历史案例运行真实 Document / Market / Model 链路。
...
两组启动命令最终都进入 `app/streamlit_app.py`，并在启动前执行 runtime / clone-ready preflight；失败时不会继续启动半可用界面。
...
- [`docs/FRONTEND_JUDGE_FACING_HANDOFF.md`](docs/FRONTEND_JUDGE_FACING_HANDOFF.md) — 统一评审入口与界面真相
